...
class SetProvider extends ChangeNotifier {
  ...
  SetProvider(this.userEmail) {
    if (userEmail.isNotEmpty) {
      _setCollection = _firestore
          .collection('users')
          .doc(userEmail)
          .collection('sets-sprint2');
    }
  }
  ...
  QuestionSet getSetByName(String setName) {
    try {
      return _sets.firstWhere((set) => set.setName == setName,
          orElse: () =>
              QuestionSet(setName: '', isActive: false, questions: []));
    } catch (e) {
    ...
    try {
      _firestoreSubscription =
        ...
        for (final doc in snapshot.docs) {
          setFutures.add(_fetchSetWithQuestions(
            doc,
          ));
        }
  ...
  Future<QuestionSet> _fetchSetWithQuestions(DocumentSnapshot doc) async {
    try {
      ...
      debugPrint(
          "fetchSetWithQuestions: Error fetching questions for set ${doc.id}: $e");
      return QuestionSet(
        id: doc.id,
        ...
        questions: [],
        selectedColorKey:
            ColorSchemeKey.Default, // Replace with a default value
      );
    }
  }
    ...
    for (int i = 0; i < oldSets.length; i++) {
      ...
      for (int j = 0; j < oldSets[i].questions.length; j++) {
        if (oldSets[i].questions[j].id != newSets[i].questions[j].id ||
            oldSets[i].questions[j].question !=
                newSets[i].questions[j].question ||
            oldSets[i].questions[j].correctAnswer !=
                newSets[i].questions[j].correctAnswer ||
            oldSets[i].questions[j].wrongAnswers.length !=
                newSets[i].questions[j].wrongAnswers.length ||
            oldSets[i].questions[j].explanation !=
                newSets[i].questions[j].explanation ||
            oldSets[i].questions[j].correctAnswers !=
                newSets[i].questions[j].correctAnswers ||
            oldSets[i].questions[j].totalAnswers !=
                newSets[i].questions[j].totalAnswers ||
            oldSets[i].questions[j].setName !=
                newSets[i].questions[j].setName) {
          return false;
        }
        for (int k = 0; k < oldSets[i].questions[j].wrongAnswers.length; k++) {
          if (oldSets[i].questions[j].wrongAnswers[k] !=
              newSets[i].questions[j].wrongAnswers[k]) {
            return false;
          }
        }
      }
  ...
  Future<void> updateSetColorAndName(
      String setName, ColorSchemeKey colorKey, String newName) async {
    try {
      late String setId;
      final setIndex = _sets.indexWhere((set) => set.setName == setName);
      if (setIndex == -1) {
      ...
      final questionsSnapshot =
          await _setCollection.doc(setId).collection('questions').get();
  ...
  Future<void> updateQuestionSuccessRate(
      Question question, bool success) async {
    try {
      late String setId;
      final setIndex =
          _sets.indexWhere((set) => set.setName == question.setName);
      if (setIndex == -1) {
      ...
      final batch = _firestore.batch();
      final questionRef =
          _setCollection.doc(setId).collection('questions').doc(question.id);
      ...
      // Update the local _sets with the new question values
      final qIndex = _sets[setIndex]
          .questions
          .indexWhere((q) => q.id == question.id);
      if (qIndex != -1) {
        _sets[setIndex].questions[qIndex].lastAnswered = DateTime.now();
      }

      notifyListeners();
    } catch (e) {
      debugPrint(
          "updateQuestionSuccessRate: Error updating question success rate: $e");
    }
  }
  ...
  Future<void> createSet(String name, ColorSchemeKey colorKey) async {
    try {
      ...
      final setIndex =
          _sets.indexWhere((setCompare) => setCompare.id == set.id);
      if (setIndex == -1) {
        _sets.add(set);
      }
  ...
  Future<void> addQuestionToSet(String setName, Question newQuestion) async {
    try {
      ...
      final setRef = _setCollection.doc(setId);
      final docRef =
          await setRef.collection('questions').add(newQuestion.toMap());
  ...
  Future<void> updateQuestionInSet(
      String setName, Question editedQuestion) async {
    try {
      late String setId;
      final setIndex = _sets.indexWhere((set) => set.setName == setName);
      if (setIndex == -1) {
      ...
      final questionIndex = _sets[setIndex]
          .questions
          .indexWhere((q) => q.id == editedQuestion.id);
      _sets[setIndex].questions[questionIndex] = editedQuestion;
      setId = _sets[setIndex].id!;
      final setRef = _setCollection.doc(setId);
      if (questionIndex < 0 ||
          questionIndex >= _sets[setIndex].questions.length) {
